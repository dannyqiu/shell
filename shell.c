...
node * parse_input(char *input, node * path) {
  ...
  while (input[index]) {
	...
	if (args != 0 || tokIndex != 0) { // Makes sure that there is something to execute
	  ...
	  add_null_argv();
	  path = execute(argv, BG_PROCESS,path);
	  cleanup_argv();
	  setup_argv();
	...
	if (pipe(pipes) == -1) { // If pipe creation fails, print error
	  ...
	  path = execute(argv, FG_PROCESS, path);
...
# ifdef DEBUG
    printf("Memory PATH IN: %x\n",path);
    printf("Path PATH IN: %s\n",path->arg);
# endif
    path = change_directory(argv[1] , path); // Only parse the first argument in cd
# ifdef DEBUG
    printf("memory PATH OUT: %x\n", path);
    printf("path PATH OUT: %s\n", path->arg);
# endif
  }
...
# ifdef DEBUG
  printf("PATH: %s\n",path);
# endif
  int hack = 0;//Quick Hack to get the right history writing
  if (!path) { // When no path specified, use home
    path = getenv("HOME");
  }
  
  else if (path[0] == '-') {
    history = get_prev(history);
    path = history->arg;
    hack = 1;
  }
# ifdef DEBUG
...
# endif
  ...
  if (errno_result == -1) {
    ...
    char * path_input = strdup(current_path); 
    history = insert_node(history, path_input);
  }
# ifdef DEBUG
